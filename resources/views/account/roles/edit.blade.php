...
<div class="mb-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-xl sm:text-xl font-bold text-gray-900">Edit Role</h2>
            <p class="text-gray-600 mt-1 italic">Update the role and its permissions</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden mt-5">
        <div class="px-6 py-6 border-b border-gray-200">
            ...
            <form method="POST" action="{{ route('account.roles.update', $role->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $role->name) }}" placeholder="Role Name" class="mt-1 block w-full border border-gray-300 @error('name') border-red-500 @enderror rounded-lg px-3 py-2 focus:ring-primary focus:border-primary">
                        @error('name')
