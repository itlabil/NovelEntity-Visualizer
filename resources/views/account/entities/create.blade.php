...
<div x-data="{ entityType: '{{ old('type', 'character') }}' }" class="mb-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-xl sm:text-xl font-bold text-gray-900">Add Entity to: <span class="text-primary">{{ $novel->title }}</span></h2>
            <p class="text-gray-600 mt-1 text-xs italic">Menambahkan objek kata kunci baru yang otomatis terikat pada novel ini.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden mt-5 border border-gray-100">
        <div class="px-6 py-6">
            ...
            <form method="POST" action="{{ route('account.entities.store', $novel->id) }}">
                @csrf
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="type" x-model="entityType" class="block w-full border border-gray-300 @error('type') border-red-500 @enderror rounded-lg px-3 py-2.5 text-sm focus:ring-primary focus:border-primary font-medium bg-gray-50/50">
                                ...
                                <option value="item" {{ old('type') == 'item' ? 'selected' : '' }}>⚔️ Item</option>
                                <option value="place" {{ old('type') == 'place' ? 'selected' : '' }}>🏰 Place</option>
                            </select>
                            @error('type')
                    ...
                    <div>
                        ...
                        <input type="text" name="display_aliases" value="{{ old('display_aliases') }}" placeholder="Ex: Mok Gyeongun, Gyeongun, Asura Demon" class="block w-full border border-gray-300 @error('display_aliases') border-red-500 @enderror rounded-lg px-3 py-2.5 text-sm focus:ring-primary focus:border-primary font-medium">
                    ...
                    <div>
                        ...
                        <input type="text" name="image_url" value="{{ old('image_url') }}" placeholder="Ex: https://example.com/image.jpg" class="block w-full border border-gray-300 @error('image_url') border-red-500 @enderror rounded-lg px-3 py-2.5 text-sm focus:ring-primary focus:border-primary font-medium">
                        @error('image_url')
                            <p class="text-red-500 text-xs mt-1.5 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="desc_id" rows="3" placeholder="Tulis ringkasan info entitas dalam Bahasa Indonesia..." class="w-full bg-gray-50 border border-gray-200 @error('desc_id') border-red-500 @enderror text-sm rounded-lg px-3 py-2.5 outline-none focus:border-gray-900 font-medium placeholder-gray-400 transition-all">{{ old('desc_id') }}</textarea>
                        @error('desc_id')
                            <p class="text-red-500 text-xs mt-1.5 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="desc_en" rows="3" placeholder="Write entity description summary in English..." class="w-full bg-gray-50 border border-gray-200 @error('desc_en') border-red-500 @enderror text-sm rounded-lg px-3 py-2.5 outline-none focus:border-gray-900 font-medium placeholder-gray-400 transition-all">{{ old('desc_en') }}</textarea>
                        @error('desc_en')
                            <p class="text-red-500 text-xs mt-1.5 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
